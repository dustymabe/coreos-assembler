...
// RetryUntilTimeoutCtx calls function f until it succeeds, until
...
func RetryUntilTimeoutCtx(ctx context.Context, timeout, delay time.Duration, f func() error) error {
...
func RetryUntilTimeout(timeout, delay time.Duration, f func() error) error {
	return RetryUntilTimeoutCtx(context.Background(), timeout, delay, f)
}
